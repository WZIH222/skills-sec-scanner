...
import { severityColors, getColor } from './colors.js'

export function formatResult(result: ScanResult, filename: string): string {
  const lines: string[] = []
...
function formatScore(score: number): string {
  if (score <= 20) return chalk.green(score.toString())
  if (score <= 40) return chalk.blue(score.toString())
  if (score <= 60) return chalk.yellow(score.toString())
  if (score <= 80) return chalk.red(score.toString())
